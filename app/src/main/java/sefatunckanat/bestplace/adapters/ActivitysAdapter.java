...
public class ActivitysAdapter extends BaseAdapter{
    ...
    public ActivitysAdapter(ProfileScreen profileScreen, String[] names, int[] ids, String[] dates, String[] urls,String[] os) {
        this.name = names;
        this.id = ids;
        this.date = dates;
        this.url = urls;
        ...
        for(int i=0;i<names.length;i++){
            //Functions.log("Gelen veriler :"+name[i]+" "+id[i]+" "+date[i]+" "+olay[i]);
        }
    }
    ...
    public View getView(final int position, View convertView, ViewGroup parent) {
        ...
        boolean takipci = false;
        if(olay[position].contains("followers")){
            takipci=true;
            holder.tvText.setText(name[position]+" kişiyi takip etmeye başladı.");
        ...
        }else if(olay[position].contains("enkotu")){
            holder.tvText.setText(name[position]+" mekanı en kötü olarak oyladı.");
        }else if(olay[position].contains("enkalite")){
            holder.tvText.setText(name[position]+" mekanı en kaliteli olarak oyladı.");
            ...
            holder.tvText.setText(name[position]+" mekanı en kalabalık olarak oyladı.");
        }
        if(takipci==false){
            rowView.setOnClickListener(new OnClickListener() {
                @Override
